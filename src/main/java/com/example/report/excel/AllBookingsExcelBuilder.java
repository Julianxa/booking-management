...
public class AllBookingsExcelBuilder {
  ...
  public byte[] build(
    ...
    try (Workbook workbook = new XSSFWorkbook();
      ...
      String[] headers = TicketTypePassengerColumns.expandHeaders(ACTIVITY_HEADERS, ticketTypeNames);

      ZonedDateTime generatedAt = ZonedDateTime.now(REPORT_ZONE);
      CellStyle sectionTitleStyle = createSectionTitleStyle(workbook);
      int rowIndex =
          writeMetadata(sheet, startDate, endDate, generatedBy, generatedAt, sectionTitleStyle, headers.length);

      writeHeaderRow(sheet, rowIndex++, headers);

      ActivityTotals activityTotals = new ActivityTotals(ticketTypeNames);
      for (BookingsByActivityDateReportRow row : activityRows) {
        ...
        writeActivityDataRow(sheet, rowIndex++, row, ticketRows, ticketTypeNames, activityTotals);
      }

      writeActivityTotalRows(sheet, rowIndex, headers.length, ticketTypeNames, activityTotals);
  ...
  private int writeMetadata(
      ...
      int columnCount) {
    int row = 3;
    createLabelRow(sheet, row++, "ALL BOOKINGS BY DATE RANGE", columnCount);
    row++;
    createLabelRow(sheet, row++, "Report Start Date:", columnCount);
    createLabelRow(sheet, row++, startDate.format(REPORT_DATE_FORMAT), columnCount);
    createLabelRow(sheet, row++, "Report End Date:", columnCount);
    createLabelRow(sheet, row++, endDate.format(REPORT_DATE_FORMAT), columnCount);
    createLabelRow(sheet, row++, "Report Generated On:", columnCount);
    createLabelRow(
        sheet,
        row++,
        generatedAt.withZoneSameInstant(REPORT_ZONE).format(REPORT_GENERATED_ON_FORMAT),
        columnCount);
    createLabelRow(sheet, row++, "Generated By:", columnCount);
    createLabelRow(sheet, row++, generatedBy != null ? generatedBy : "System", columnCount);
    row++;
    createSectionTitleRow(sheet, row++, "Activity Summary", sectionTitleStyle, columnCount);
    return row;
  }
  ...
  private int writeActivityTotalRows(
    ...
    setMoneyCell(row, moneyStartColumn + 2, totals.netSale);
    ...
    ReportExcelStyles.applyGreyToEntireRow(medianRow, columnCount, true);
    return rowIndex;
  }
  ...
  private void createSectionTitleRow(
      Sheet sheet, int rowIndex, String title, CellStyle sectionTitleStyle, int columnCount) {
    Row row = sheet.createRow(rowIndex);
  ...
  private void createLabelRow(Sheet sheet, int rowIndex, String label, int columnCount) {
    Row row = sheet.createRow(rowIndex);
