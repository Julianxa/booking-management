...
public class BookingsByPurchaseDateExcelBuilder {
  ...
  /** Fallback grey-row width when gift-certificate headers are wider than activity headers. */
  private static final int GIFT_CERTIFICATE_COLUMN_COUNT = GIFT_CERTIFICATE_HEADERS.length;
  ...
  public byte[] build(
    ...
    try (Workbook workbook = new XSSFWorkbook();
      ...
      String[] activityHeaders =
          TicketTypePassengerColumns.expandHeaders(ACTIVITY_HEADERS, ticketTypeNames);
      int sheetColumnCount = Math.max(activityHeaders.length, GIFT_CERTIFICATE_COLUMN_COUNT);

      ZonedDateTime generatedAt = ZonedDateTime.now(REPORT_ZONE);
      CellStyle sectionTitleStyle = createSectionTitleStyle(workbook);
      int rowIndex =
          writeMetadata(
              sheet, startDate, endDate, generatedBy, generatedAt, sectionTitleStyle, sheetColumnCount);

      writeHeaderRow(sheet, rowIndex++, activityHeaders, sheetColumnCount);

      ActivityTotals totals = new ActivityTotals(ticketTypeNames);
      ...
      for (BookingsByActivityDateReportRow row : rows) {
        ...
        writeActivityDataRow(sheet, rowIndex++, row, ticketRows, ticketTypeNames, totals);
      }

      rowIndex =
          writeActivityTotalRows(sheet, rowIndex, sheetColumnCount, ticketTypeNames, totals);
      writeGiftCertificateSummarySection(
          sheet, rowIndex, sectionTitleStyle, giftCertificateRows, sheetColumnCount);

      for (int columnIndex = 0; columnIndex < sheetColumnCount; columnIndex++) {
  ...
  private int writeMetadata(
    ...
    int row = 2;
    createLabelValueRow(
        sheet, row++, "ALL INTERNET BOOKINGS BY DATE CREATED", null, sheetColumnCount);
    row++;
    createLabelValueRow(
        sheet, row++, "Report Start Date:", startDate.format(REPORT_DATE_FORMAT), sheetColumnCount);
    createLabelValueRow(
        sheet, row++, "Report End Date:", endDate.format(REPORT_DATE_FORMAT), sheetColumnCount);
    createLabelValueRow(
        sheet,
        row++,
        "Report Generated On:",
    ...
    createLabelValueRow(
        ...
        sheetColumnCount);
    row++;
    createSectionTitleRow(sheet, row++, "Activity Summary", sectionTitleStyle, sheetColumnCount);
    return row;
  }

  private void writeHeaderRow(Sheet sheet, int rowIndex, String[] headers, int sheetColumnCount) {
    Row row = sheet.createRow(rowIndex);
    for (int i = 0; i < headers.length; i++) {
      row.createCell(i).setCellValue(headers[i]);
    }
    ReportExcelStyles.applyGreyToEntireRow(row, sheetColumnCount, true);
  }
  ...
  private int writeActivityTotalRows(
      Sheet sheet,
      int rowIndex,
      int sheetColumnCount,
      List<String> ticketTypeNames,
      ActivityTotals totals) {
    int passengerStartColumn = 9;
    int moneyStartColumn =
        passengerStartColumn + (ticketTypeNames.isEmpty() ? 1 : ticketTypeNames.size());
    ...
    setMoneyCell(row, moneyStartColumn, totals.subtotal);
    setMoneyCell(row, moneyStartColumn + 1, totals.discount);
    setMoneyCell(row, moneyStartColumn + 2, totals.total);
    setMoneyCell(row, moneyStartColumn + 3, totals.total);
    ReportExcelStyles.applyGreyToEntireRow(row, sheetColumnCount, true);

    Row medianRow = sheet.createRow(rowIndex++);
    medianRow.createCell(5).setCellValue("Median: " + totals.medianDaysToPurchase());
    ReportExcelStyles.applyGreyToEntireRow(medianRow, sheetColumnCount, true);
    return rowIndex;
  }
  ...
  private void writeGiftCertificateSummarySection(
    ...
    createSectionTitleRow(
        sheet, rowIndex++, "Purchased Gift Certificates Summary", sectionTitleStyle, sheetColumnCount);
    writeHeaderRow(sheet, rowIndex++, GIFT_CERTIFICATE_HEADERS, sheetColumnCount);
    ...
    if (!rows.isEmpty()) {
      ...
      ReportExcelStyles.applyGreyToEntireRow(totalRow, sheetColumnCount, true);
    }
  }
  ...
  private void createSectionTitleRow(
      Sheet sheet, int rowIndex, String title, CellStyle sectionTitleStyle, int sheetColumnCount) {
    Row row = sheet.createRow(rowIndex);
  ...
  private void createLabelValueRow(
      Sheet sheet, int rowIndex, String label, String value, int sheetColumnCount) {
    Row row = sheet.createRow(rowIndex);
    row.createCell(0).setCellValue(label);
    if (value != null) {
      row.createCell(1).setCellValue(value);
    }
    ReportExcelStyles.applyGreyToEntireRow(row, sheetColumnCount, false);
  }
  ...
  private static final class ActivityTotals {
    ...
    private ActivityTotals(List<String> ticketTypeNames) {
